...
RESULT HID_Data_Setup(uint8_t RequestNo)
{
  ...
  (*CopyRoutine)(pInformation->Ctrl_Info.Usb_wOffset);
  return USB_SUCCESS;
}
...
uint8_t *HID_GetReport_Feature(uint16_t Length)
{
  ...
  {
	//запись информации в Report_buf исходя из ID репорта
	switch(pInformation->USBwValue0)
	{
		case 5:			// транспорт, пакеты размера wMaxPacketSize-3
			Report_Buf[0]=5;
			temp = RTRB((uint8_t*) &Report_Buf[3], wMaxPacketSize-4);

			//pack16to8(Report_Buf, i, p)		RB[i]=*p++; RB[i+1]=*p;	//LB uint16_t -> buffer[i] and	//HB uint16_t -> buffer[i+1]
			Report_Buf[1] = *pB++;
			Report_Buf[2] = *pB;
	    break;
		case 6:			// запись в буффер репорта данных о размере выходного буффера
			Report_Buf[0]=6;
			temp = GetSizeRingBuf();
			pack16to8(Report_Buf, 1, pB);
			Report_Buf[1] = *pB++;
			Report_Buf[2] = *pB;
	    break;
		default:
